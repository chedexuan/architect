...
local function fail(code, msg, detail)
...
--
-- 1. 2.0 exposes the draw through a getter; the `energy_usage` attribute is nil for
--    inserters and belts, so reading only the attribute silently drops a large share of a
--    card's demand -- a stack inserter costs more than an assembler.
-- 2. The getter answers in J/tick, not kW, while the prototype data states kW. Every power
...
local function kw_of(j_per_tick)
...
local function world_db()
  ...
  for name, p in pairs(prototypes.entity) do
    if field(p, "type") == "resource" then
      raw[name] = { category = field(p, "resource_category") or field(p, "category") }
    end
  end
...
local function resolve_surface(spec)
...
function M.card_lab(args)
  ...
  if next(contract) == nil then
    return fail("CARD_NO_CONTRACT", "contract.outputs is required; without a claim to check there is no verdict")
  end
  ...
  local origin, rejected = find_card_site(surface, normalized, force_name, args.origin)
  if not origin then
    return fail("NO_CLEAR_SITE", "no candidate site fits this card; pass an explicit origin", rejected)
  end
  ...
  local fuel = args.fuel == false and nil or (args.fuel or "coal")
  local fuel_targets = {}
  ...
  for _, port in ipairs(ports_out) do
    local rec = built[port.entity]
    if rec then collect[#collect + 1] = { entity = rec.entity, item = port.item } end
  end
  if #feeds == 0 then
    verify.destroy(built)
  ...
  for idx, rec in pairs(built) do
    if rec.kind == "assembling-machine" then
      ...
      if not want then
        ...
        for item in pairs(contract) do
          local list = candidates_for(item)
          if #list == 1 then
            want = list[1]
            break
          elseif #list > 1 then
  ...
  storage.lab = {
    ...
    ingredient = feeds[1].item,
    product = (ports_out[1] or {}).item,
    produced = 0,
    fed = 0, fed_blocked = 0,
...
local finish_drill_job, step_drill_job
...
function M.drill_rate(args)
  ...
  if args.refresh ~= true and storage.drills[key] then
    local c = storage.drills[key]
    c.cached = true
    return c
  end
  ...
  for dx = -3, 3 do
    for dy = -3, 3 do
      ...
      if surface.can_place_entity { name = machine, position = pos, force = force_name } then
        drill = surface.create_entity { name = machine, position = pos, force = force_name }
        if drill then at = pos break end
      end
    end
  ...
  for i = 0, (args.belt_tiles or 4) - 1 do
    ...
    local b = surface.create_entity { name = "transport-belt", position = pos, force = force_name, direction = dir }
  ...
  return { state = "running", machine = machine, resource = resource, seconds = seconds,
           output_belt = #belt_units, died = storage.drill_dead, runner = storage.drill_error,
...
local function find_rig_entity(surface, unit, name, pos)
...
function M.pump_rate(args)
  ...
  local gen, attempts = nil, {}
  if args.supply then
    pcall(function() surface.create_global_electric_network() end)
    for _, o in ipairs({ 4, 5, 6, 8, 10 }) do
      local pos = { x = pump.position.x, y = pump.position.y + o }
...
function finish_pump_job()
  ...
  local tank_capacity = prototypes.entity["storage-tank"]
    and field(prototypes.entity["storage-tank"], "fluid_capacity")
  ...
  storage.pumps[j.key] = {
    ...
    tank_ever_at_capacity = tank_capacity and (function()
      for _, seen in pairs(j.fluids) do if (seen.held or 0) >= tank_capacity then return true end end
    end)(),
...
script.on_nth_tick(1, function()
  drive_measurement(step_drill_job, "drill_job", "drill_dead", "drill_error", reap_rig)
  drive_measurement(step_pump_job, "pump_job", "pump_dead", "pump_error", reap_parts)
